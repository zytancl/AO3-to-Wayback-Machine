...
lately, this script is unable to work properly as intended. *ao3 is actively blocking all of wayback machine's crawler eps at the network level and this is something that i'm currently unable to bypass.* as of now, until I can finally fix this and get this to work properly, i'm removing the file and download link for this. if you have already installed this, i also suggest that you disable/uninstall this for now!
...
- **Extension-like archiving** - when you are logged into the Internet Archive in your browser, the script uses your session exactly the way the Wayback Machine browser extension does by attempting to give its best chance of a successful save
...
The script updates automatically via your userscript manager. Tampermonkey and Violentmonkey check the `@updateURL` daily and prompt you when a new version is available. You can also right-click the script and choose **Check for updates** to check immediately or click on the **Install** below to reinstall the script!
...
The script works out of the box, but logging into the Internet Archive makes archiving significantly more reliable, it is the same thing the Wayback Machine browser extension does.

### Step 1 — Create/login a free Internet Archive account

Go to https://archive.org/, create a free account, verify your email, and ensure that you have logged in.

### Step 2 — Confirm the script sees your session

1. Go to any AO3 work or series page
2. Click the ⚙ button at the bottom-right corner of the page
3. Check the status badge at the top of the settings panel: Green — logged in, using browser session (best mode, identical to the WM extension); Yellow — not logged in, using API keys or anonymous mode

### Optional step 3 — S3 API keys

S3 API keys are only needed if you are not logged into Internet Archive in your browser. If you have already logged in, the session is used automatically and keys are not required. However, you can still do this just as a pre-caution in case you may have unknowingly logged out.

If you want to use API keys as a fallback:

1. Go to archive.org/account/s3.php
2. Click Generate new keys if none are shown
3. Copy your Access Key and Secret Key
4. Open the ⚙ settings panel on any AO3 page and paste them into the Internet Archive fields
...
The script uses the **Save Page Now 2 (SPN2) API** to send AO3 fics to the Wayback Machine. The auth method used determines the quality of the save:
 
| Mode | When used | Notes |
|---|---|---|
| **Browser session** | Logged into archive.org in the browser | Identical to the Wayback Machine extension. Best results. |
| **S3 API keys** | Not logged in, but keys are entered in settings | Programmatic API mode. Works but may be less reliable for bot-blocked sites. |
| **Anonymous** | Neither session nor keys | Rarely succeeds for AO3 due to AO3's crawler blocking. |
 
### With session / API key mode
 
1. You submit a bookmark on AO3
2. The script makes a POST to `https://web.archive.org/save` with your AO3 cookies and IA auth
3. Wayback queues a headless-browser capture job (`capture_screenshot=on`)
4. The script polls the job status every 5 seconds (up to 5 minutes)
5. Banners update as the job progresses and show the final result
6. If archiving completed while you were navigating to a new page, the result banner appears on the next AO3 page you visit

### Tab fallback / no IA credentials
 
1. A background tab opens to `https://web.archive.org/save/[url]`
   - On **mobile / tablet** the tab opens in the foreground (background tabs are suspended before Wayback can load)
2. The tab closes after 30 seconds (60 on mobile)
3. After 35 seconds the script checks the Wayback CDX API for a snapshot
4. If none is found, the script retries with a simpler URL:
   - Attempt 1 — `?view_adult=true&view_full_work=true`
   - Attempt 2 — `?view_adult=true` only
   - Attempt 3 — base URL, no parameters

> Note: AO3 blocks some unauthenticated crawlers at the server level, which can cause Wayback to receive a "job failed" or 404 even for fully public fics. **This is an AO3-side restriction the script cannot work around.** Retries help in cases where AO3 is temporarily rate-limiting, but persistent 404s from Wayback mean the save did not go through. This is something I can't do anything about it, that's why I kept changing the code in the beginning cause I was desperately trying to bypass it. Logging in and/or adding your IA API keys (above) is the reliable fix.
...
- The session-based mode (`GM_cookie` reading archive.org cookies) requires Tampermonkey, but Violentmonkey and Userscripts (Safari) will fall back to S3 keys or anonymous mode.
