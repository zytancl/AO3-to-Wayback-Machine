...
| Max retries | `2` | How many times to retry if Wayback does not save the fic on the first attempt (0 to disable, max 5) |
| Retry delay | `20s` | How long to wait between retry attempts (10–120 seconds) |
...
- There is a chance that you may get rate-limited by Wayback Machine if you have been bookmarking constantly in a short while or you're bookmarking a series that contains many works, you can either try again later, but the worse case scenario is that the only solution is for you to archive them yourself.
